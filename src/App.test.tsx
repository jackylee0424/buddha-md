...
describe('App', () => {
  ...
  it('renders the clean book-centered landing page by default', () => {
    ...
    expect(screen.getByRole('heading', { name: '讓AI Agent來回答六祖壇經講記相關問題，複製後貼給你的AI Agent', level: 4 })).toBeInTheDocument();
    expect(screen.getAllByRole('button', { name: '展開', expanded: false })).toHaveLength(2);
    expect(screen.queryByText(/curl -fsSL https:\/\/buddha\.md\/install\.sh \| sh/)).not.toBeInTheDocument();
    fireEvent.click(screen.getAllByRole('button', { name: '展開' })[0]);
    expect(screen.getByRole('button', { name: '收合', expanded: true })).toBeInTheDocument();
    expect(screen.getByText(/curl -fsSL https:\/\/buddha\.md\/install\.sh \| sh/)).toBeInTheDocument();
    expect(screen.getByText(/buddha-md --help/)).toBeInTheDocument();
    expect(screen.getByText(/buddha-md fetch diamond/)).toBeInTheDocument();
    expect(screen.queryByText(/buddha-md fetch tangjin/)).not.toBeInTheDocument();
